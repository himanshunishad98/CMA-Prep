    ...
    if (data.status === 'success') {
      _loadedTests = (data.data?.tests || data.tests || [])
        ...
        .map(t => ({
          ...
          price:           t.price  !== undefined ? t.price  : (t.pricing ? t.pricing.price  || 0 : 0),
          prizes:          t.prizes || { first: 0, second: 0, third: 0 },
          prize1:          t.prize1 !== undefined ? t.prize1 : (t.prizes ? t.prizes.first  || 0 : 0),
          prize2:          t.prize2 !== undefined ? t.prize2 : (t.prizes ? t.prizes.second || 0 : 0),
          prize3:          t.prize3 !== undefined ? t.prize3 : (t.prizes ? t.prizes.third  || 0 : 0),
...
function renderTestCards() {
  ...
  _loadedTests.forEach(test => {
    ...
    // Price: check Pricing sheet testType first, then fall back to flat price from JSON_DATA
    const pricingObj = test.pricing || {};
    const flatPrice  = test.price  || 0;
    let priceText;
    if (pricingObj.testType === 'Paid' && pricingObj.price > 0) {
      priceText = `₹${pricingObj.price}`;
    } else if (flatPrice > 0) {
      priceText = `₹${flatPrice}`;
    } else {
      priceText = 'FREE';
    ...
    // Prizes: only render row when at least one prize is non-zero
    const p1 = test.prize1 || 0, p2 = test.prize2 || 0, p3 = test.prize3 || 0;
    const prizeHtml = (p1 || p2 || p3)
      ? `<div style="display:flex;justify-content:center;gap:16px;margin-top:16px;padding-top:12px;border-top:1px solid #E2E8F0;font-size:11.5px;color:var(--text2);">
           ${p1 ? `<span>🥇 ₹${p1}</span>` : ''}
           ${p2 ? `<span>🥈 ₹${p2}</span>` : ''}
           ${p3 ? `<span>🥉 ₹${p3}</span>` : ''}
         </div>`
      ...
      <div class="test-card-footer flex-between">
        ...
        ${test.status === 'upcoming'
          ? `<div style="display:flex;align-items:center;gap:8px;">
               <span style="font-size:11.5px;padding:5px 10px;border-radius:6px;background:#EFF6FF;color:#1D4ED8;font-weight:600;border:1px solid #BFDBFE;white-space:nowrap;">🔔 Upcoming</span>
               <button class="btn btn-navy btn-sm" style="white-space:nowrap;"
...
async function _renderLevel2Panel() {
  ...
  try {
    ...
    if (data.status === 'success' && data.data?.level2) {
      ...
      // Also update the profile page Level 2 status block (Issue 2 & 11)
      const pStatus = document.getElementById('profile-l2-status');
      const pDate = document.getElementById('profile-l2-date');
      const pLink = document.getElementById('profile-l2-link');
      if (pStatus) {
        pStatus.textContent = l2.status || 'Not Qualified';
        pStatus.className = 'a-badge ' + (l2.status === 'Live' ? 'a-badge-live' : (l2.status === 'Upcoming' ? 'a-badge-pending' : 'a-badge-draft'));
      }
      if (pDate) pDate.textContent = l2.date ? new Date(l2.date).toLocaleDateString() : '—';
      if (pLink) {
        // Issue 11: Granular control over visibility
        if (l2.meetLink && (l2.status === 'Upcoming' || l2.status === 'Live')) {
          pLink.innerHTML = `<a href="#" onclick="window.open('${l2.meetLink}', '_blank'); return false;" style="color:var(--navy3)">Join Link</a>`;
        } else {
          pLink.textContent = '—';
        }
      }
      ...
      if (btn) {
        if (l2.status === 'Live' && l2.meetLink) {
          btn.disabled = false;
          // Issue 9: Always fetch latest status before actually joining
          btn.onclick = async () => {
             btn.textContent = 'Joining...';
             const freshData = await apiCall('getLevel2Status');
             if (freshData.status === 'success' && freshData.data?.level2?.status === 'Live' && freshData.data?.level2?.meetLink) {
                window.open(freshData.data.level2.meetLink, '_blank');
             } else {
                showToast('Meeting is no longer live.');
                _renderLevel2Panel(); // Re-render to reflect new state
             }
...
window.toggleNotifications = function() {
  ...
  if (panel) {
    // Issue fix: Use classList to trigger CSS animation instead of inline styles
    panel.classList.toggle('open');
    panel.style.display = ''; // Clear any rogue inline styles
  }
};
