...
function createTest(data) {
  ...
  // Store full testData blob if provided
...
function getAllTests(data) {
  ...
  if (pSheet && pSheet.getLastRow() >= 2) {
    const pRows = pSheet.getRange(2, 1, pSheet.getLastRow() - 1, 7).getValues();
    pRows.forEach(r => {
      pricingData[r[0]] = {
        price:        r[2],
        discountPrice: r[3],
  ...
  const tests = rows
    ...
    .map(r => {
      ...
      return {
        ...
        duration:       r[4],
        totalQuestions: r[5],
        pricing:        pricingData[r[0]] || { testType: 'Free', price: 0 },
        testData:       isAdmin ? testJson : {},   // never send questions to public
        status:         testJson.status || 'draft', // default draft — must be explicitly set live/upcoming
        startTime:      testJson.startTime || null,
        endTime:        testJson.endTime || null
      };
    })
    .filter(t => isAdmin || t.status === 'live' || t.status === 'upcoming'); // public only sees live/upcoming
...
function fetchTests(payload) {
    ...
    // ── Pricing map (admin only, avoids unnecessary sheet reads) ──
    const pricingMap = {};
    if (isAdmin) {
      const ss     = SpreadsheetApp.openById(SHEET_ID);
      const pSheet = ss.getSheetByName(PRICING_SHEET_NAME);
    ...
    const tests = rows
      ...
      .map(r => {
        ...
        const record = {
          ...
          testType:       (pricingMap[testId] || {}).testType || 'Free',
        ...
        if (isAdmin) {
          record.pricing  = pricingMap[testId] || { testType: 'Free', price: 0 };
        }
